...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,io.github.Nanshenger.DreamY";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("io.github.Nanshenger.DreamY", 1, 9, "DreamY", "1.0.4", 4, "Nanshenger", "南笙机器人", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("io.github.Nanshenger.DreamY", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("io.github.Nanshenger.DreamY", AppData.CQLog);	
